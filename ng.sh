...
clean_ssl_session_cache() {
  ...
  for file in "${paths[@]}"; do
    if [ -f "$file" ]; then
      sed -i '/ssl_session_cache/d' "$file"
...
detect_conf_path() {
  ...
  # 檢查是否已有 include *.conf
  include_lines=$(sed -n '/http[[:space:]]*{/,/^}/p' "$nginx_conf" | tr -d '\r' | grep -E 'include[[:space:]]+[^;]*\*\.conf;')
  while IFS= read -r line; do
    raw_path=$(echo "$line" | sed -E 's/^[[:space:]]*include[[:space:]]+(.+\*\.conf);/\1/')
    raw_path=$(echo "$raw_path" | sed -E 's|/\*\.conf$||')
    raw_path=$(eval echo "$raw_path")
    [ -z "$raw_path" ] && continue

    resolved_path=$(realpath "$raw_path" 2>/dev/null)
    [ -z "$resolved_path" ] && resolved_path=$(dirname "$raw_path")
    echo "$resolved_path"
    return 0
  done <<< "$include_lines"
  ...
  # 若沒有 include，就新增
  if command -v openresty >/dev/null 2>&1; then
    local conf_path="/usr/local/openresty/nginx/conf/conf.d"
  elif command -v nginx >/dev/null 2>&1; then
    local conf_path="/etc/nginx/conf.d"
  fi

  if ! grep -q "include $conf_path/*.conf;" "$nginx_conf"; then
  # 找到 http { 的下一行插入 include
    sed -i '/^\s*http\s*{/a \    include '"$conf_path"'/*.conf;' "$nginx_conf"
  fi
  mkdir -p $conf_path
    ...
    1|2|3)
      case $type in
        html|php|www|flarum)
          ...
          sed -i "s|main|$escaped_cert|g" "$conf_file"
...
manage_cf_origin_cert(){
  echo "Cloudflare 原始證書管理"
  echo "1. 申請原始證書"
  echo "2. 刪除原始證書"
  echo "0. 返回"
  read -p "請選擇: " cf_choice
  case "$cf_choice" in
    1) 
      cf_cert_autogen
      read -p "操作完成，請按任意鍵繼續..." -n1
...
menu_ssl_apply() {
  ...
  echo -e "${YELLOW}如果您是要申請cloudflare原始憑證，請至7. 管理 Cloudflare 原始證書${RESET} "
...
menu_ssl_revoke() {
  ...
  echo -e "${YELLOW}如果您是要吊銷cloudflare原始憑證，請至7. 管理 Cloudflare 原始證書${RESET} "
...
while true; do
  ...
  case $choice in
    ...
    6)
      ...
      ;;
    7)
      manage_cf_origin_cert || continue
      ;;
    0)
      exit 0
      ;;
